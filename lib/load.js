...
var path          = require('path')
  ...
  , loadModuleInContext = require('next/lib/vm/load-module-in-context')
...
ctxRequireSilent = function (path, context) {
	try {
		return loadModuleInContext(path, context);
	} catch (e) {
		return (e.message === "ENOENT, No such file or directory '" + path + "'") ? null : e;
	}
};


module.exports = function (testeePath, testPath, context, withcoverage) {
	var d, o, contextRequire;
	d = deferred();
	o = {};
...
// 	if (context !== global) {
	if (false) {
		o.testee = ctxRequireSilent(testPath, context);
		o.test   = ctxRequireSilent(testPath, context);
