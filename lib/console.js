...
var call      = Function.prototype.call
  ...
  , flog      = require('es5-ext/lib/Function/prototype/log')
  , peek      = require('es5-ext/lib/Array/prototype/peek')
  , indent    = call.bind(curry.call(require('es5-ext/lib/String/get-indent')(' '), 17, true))
  , format    = call.bind(require('es5-ext/lib/Date/get-format')('%H:%M:%S.%L '))
...
o = {
	init: function (options) {
		...
		this.writeLog = [];
		this.write = flog.call(write, this.writeLog);
		cthrobber(this.progress = interval(200, true));
		return this;
	},
	atNewLine: function () {
		return !this.writeLog.length ||
			peek.call(ctrim(peek.call(this.writeLog)[0])) === '\n';
	},
	break: function () {
	...
	error: function (path, name, e) {
		...
		if (!all) {
			...
			this.write(lsummary(dpad(duration.call(this.started, new Date).toString())
				+ '     '));
			s = [];
